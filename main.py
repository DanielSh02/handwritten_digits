...
# place the name of the source file for the trained network here.
src = "pickles/100-20.pickle"
...
def on_draw():
    ...
    if canvas.result is not None:
        result_text = pyglet.text.Label(f'Did you write a {str(canvas.result)}? \n Confidence: {canvas.confidence}',
    ...
    for i in range(max(0, round(x - brush_radius)), min(HEIGHT - 1, round(x + brush_radius))):
        for j in range(max(0, round(y - brush_radius)), min(WIDTH - 1, round(y + brush_radius))):
            ...
            if d ** 2 < brush_radius ** 2:
                canvas.grid[i][j] = min(pen_eraser,
                                        canvas.grid[i][j] + pen_eraser / d ** 3)  # * math.exp(-d ** 2 * 0.1)
