from numpy import array, add, argmax, zeros, multiply
...
class Network:
    def __init__(self, hidden_layers, labels, raw, learning_rate = 0.1, batch_size = 20, print_statements = True):
        ...
        # Weights and biases
        self.weights = [array([])] + [layer.weights for layer in self.layers[1:]]
        self.biases = [array([])] + [layer.biases for layer in self.layers[1:]]
        ...
        for i in range(epochs):
            ...
            while True:
                ...
                if not self.data_set.idx % self.batch_size:
                    self.backprop()
                    if self.print_statements and not (self.data_set.idx // self.batch_size) % 500:
                        print(f'Epoch: {i}')
    ...
    def calc_change(self, actual):
        ...
        dCdz = multiply(dsig(self.zs[-1]), self.layers[-1].nodes - desired)
        bias_changes[-1] = dCdz
        weight_changes[-1] = dCdz @ self.layers[-2].nodes.transpose()
        for i in range(n - 2, 0, -1):
            z = self.zs[i]
            dCdz = multiply(dsig(z), self.weights[i + 1].transpose() @ dCdz)
            weight_changes[i] = dCdz @ self.layers[i - 1].nodes.transpose()
            bias_changes[i] = dCdz
    ...
    def backprop(self):
        ...
        self.b_batch = []
        if self.print_statements:
            batch_no = self.data_set.idx // self.batch_size
            if not batch_no % 500:
                print(f"Batch {self.data_set.idx // self.batch_size} average cost: {self.batch_cost / self.batch_size}")
        self.batch_cost = 0
    ...
    def test(self, raw):
        ...
        while True:
            try:
                data, ans = next(dataset)
                self.run(data)
                if ans[0] == self.layers[-1].result:
                    correct += 1
            except:
                break
        print(f'{correct} correct out of {len(dataset)} cases')

class Layer:
    def __init__(self, num_nodes):
        self.num_nodes = num_nodes
        self.nodes = array([0 for i in range(num_nodes)]).transpose()
...
class HiddenLayer(Layer):
    def __init__(self, num_nodes, prev_layer):
        ...
        def randizer():
            return normal(scale = self.prev_layer.num_nodes ** (-0.5))
